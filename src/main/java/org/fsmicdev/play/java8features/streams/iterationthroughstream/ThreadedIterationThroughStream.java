package org.fsmicdev.play.java8features.streams.iterationthroughstream;
 ...
 *
 *
 * Parallel computing involves dividing a problem into sub-problems,
 * solving those problems simultaneously (in parallel, with each sub-problem running in a separate thread),
 *  and then combining the results of the solutions to the sub-problems.  Java SE provides the fork/join framework,
 *  which enables you to more easily implement parallel computing in your applications.  However, with this framework,
 *  you must specify how the problems are sub-divided (partitioned).
 *  With aggregate operations, the Java runtime performs this partitioning and combining of solutions for you.
 *
 * Limit the parallelism that the ForkJoinPool offers you. You can do it yourself by supplying the
 ...
 * @author mic
 */
public class ThreadedIterationThroughStream {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        Thread t2 = new Thread(ThreadedIterationThroughStream:: internalIterationOnThread);
        Thread t3 = new Thread(ThreadedIterationThroughStream:: internalIterationOnThread);
        Thread t4 = new Thread(ThreadedIterationThroughStream:: internalIterationOnThread);
        ...
        smallListOfNumbers.parallelStream().forEach(
                (Integer s) -> {
                    try {
                         ...
                         */
                        Thread.sleep(100); // Recommend: Set a break-point here, and see debugging state.
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
